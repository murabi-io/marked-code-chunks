...
type TestCaseType = {
  attributes?: Record<string, NodeValue>;
  normalizedAttributes?: Record<string, NodeValue>;
  raw?: string | string[];
  stringified?: string;
};
...
describe("block-attributes", () => {
  testCases.map(({ raw, attributes }) => {
    ...
    arrayOfTexts.map((text) => {
      it(`parseBlockAttributes() correctly parses ${text}`, () => {
  ...
  testCases.map(({ attributes = null, normalizedAttributes = null }) => {
    ...
    it(`normalizeBlockAttributes() correctly normalizes ${JSON.stringify(
      attributes
    )}`, () => {
  ...
  testCases.map(
    ({
      ...
      attributesToTest.map((attrs) => {
        it(`stringifyBlockAttributes() correctly stringifies ${JSON.stringify(
          attrs
        )}`, () => {
